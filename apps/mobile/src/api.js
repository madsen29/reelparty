...
import { defaultTitle, ytThumb } from "./lib";
...
const mapVideo = (r) => ({
  ...
  watchCount: Array.isArray(r.watched_by)
    ? r.watched_by.length
    : r.watched
      ? 1
      : 0,
...
export async function removeMember(code, memberId, hostId) {
  await req(
    `/api/parties/${encodeURIComponent(code)}/members/${encodeURIComponent(memberId)}/remove`,
    {
      method: "POST",
...
export async function fetchMeta({ platform, videoId, url }) {
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 10000);
    const res = await fetch(
      `${API_BASE_URL}/api/meta?url=` + encodeURIComponent(url),
      { signal: ctrl.signal },
    );
    clearTimeout(t);
    if (res.ok) {
